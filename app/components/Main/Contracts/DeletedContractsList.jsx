...
export default function DeletedContractsList({ lang }) {
    // ROUTER
    ...
    // GET THE DELETED CONTRACTS FROM THE API
    function getDeletedContracts() {
        const token = localStorage.getItem('token');

        axios
            .get(`${process.env.API_URL}/deleted/contracts`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
            .then((res) => {
                // The API returns removedContracts array, we need to adapt it to our component structure
                const removedContracts = res.data?.removedContracts || [];
                // Transform the data to match the expected structure
                const transformedData = removedContracts.map(contract => ({
                    ...contract,
    ...
    // EFFECT TO GET THE DELETED CONTRACTS
    useEffect(() => {
        getDeletedContracts();
    }, []);

    // ADD PAYMENT HANDLER (for deleted contracts that might be restored)
    ...
    return (
        <>
            <DataTable
                ...
                emptyMessage={lang === 'en' ? 'No deleted contracts found.' : 'لا يوجد عقود محذوفة.'}
                dir={lang === 'ar' ? 'rtl' : 'ltr'}
            >
            ...
            <Dialog maximizable onHide={() => setSelectedItem(null)} visible={!!selectedItem} header={lang === 'en' ? 'Deleted Contract Details' : 'تفاصيل العقد المحذوف'} style={{ width: '90vw' }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
